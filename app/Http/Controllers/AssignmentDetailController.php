...
class AssignmentDetailController extends Controller
{
    ...
    public function submitAssignmentAnswer(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            ...
            $file_name = 'SUB_ASG_' . $request->assignment_title . '_' . auth()->user()->user_id . '_' . time() . '.' . $extension;
        ...
        AssignmentDetail::create([
            'assignment_header_id' => $request->assignment_header_id,
            'user_id' => auth()->user()->user_id,
            'file' => $file_name
        ]);

        // Students submit assignment set score back to null
        AssignmentScore::where([['assignment_header_id', $request->assignment_header_id],['user_id', auth()->user()->user_id]])
            ->update([
                'score' => null
            ]);
